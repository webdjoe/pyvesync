...
API_TIMEOUT = 8
USER_AGENT = (
    "VeSync/3.2.39 (com.etekcity.vesyncPlatform;"
...
class Helpers:
    ...
    def req_body_base(manager) -> dict:
        ...
        return {"timeZone": manager.time_zone, "acceptLanguage": "pt"}
    ...
    def bypass_body_v2(cls, manager):
        ...
        bdy["userCountryCode"] = manager.country_code
        return bdy
...
class Color:
    ...
    @staticmethod
    def min_max(
        value: Union[int, float, str], min_val: float, max_val: float, default: float
    ) -> float:
    ...
    def valid_hsv(
        ...
        return (valid_hue, valid_saturation, valid_value)
...
class Timer:
    ...
    def _internal_update(self) -> None:
        ...
        if self._status == "paused":
            self.update_time = None
            return
        if self._status == "done" or (
            self._seconds_since_check > self._remain and self._status == "active"
        ):
            self._status = "done"
            self.update_time = None
            self._remain = 0
    ...
    def update(
        self, *, time_remaining: Optional[int] = None, status: Optional[str] = None
    ) -> None:
        """Update timer.
