...
class VeSync:  # pylint: disable=function-redefined
    ...
    def __init__(
        self, username, password, time_zone=DEFAULT_TZ, debug=False, redact=True
    ):
    ...
    def debug(self, new_flag: bool) -> None:
        """Set debug flag."""
        log_modules = [bulb_mods, switch_mods, outlet_mods, fan_mods, helpermodule]
        if new_flag:
            logger.setLevel(logging.DEBUG)
    ...
    def login(self) -> bool:
        ...
        response, _ = Helpers.call_api(
            "/cloud/v1/user/login", "post", json_object=Helpers.req_body(self, "login")
        )
