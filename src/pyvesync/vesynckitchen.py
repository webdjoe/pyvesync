...
from pyvesync.helpers import APP_VERSION, PHONE_BRAND, PHONE_OS
...
class VeSyncAirFryerCAF(VeSyncBaseDevice):
    ...
    def _cookmode_apiV2(self, method_cmd: dict):
        ...
        if method_cmd["cookMode"].get("cookStatus") == "endCook":
            ...
            cmd = body["payload"].get("data")

            if self.temp_unit.lower() in ["f", "fahrenheit"]:
                cmd.update(
                    {
                        "tempUnit" : "f"
                    }
                )
            elif self.temp_unit.lower() in ["c", "celsius"]:
                cmd.update(
                    {
                        "tempUnit" : "c"
                    }
...
class VeSyncAirFryer158(VeSyncBaseDevice):
    ...
    def end(self):
        """End the cooking process."""
        if self.preheat is False and self.fryer_status.cook_status in [
            "cookStop",
            "cooking",
        ]:
            cmd = {"cookMode": {"cookStatus": "end"}}
        elif self.preheat is True and self.fryer_status.cook_status in [
            "preheatStop",
            "heating",
        ]:
